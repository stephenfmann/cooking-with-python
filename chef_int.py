...
class Chef:
	...
	def put(self, mixingbowl, value):
		...
		if mixingbowl == None:
			...
			if not self.mixingbowls.has_key(key):
		...
		for ex in excode:
			...
			if clean != None:
				if clean.group(1) == None:
					...
					if self.mixingbowls.has_key(clean.group(1)[:-2]):
						...
						if existslater == None:
							print "Warning: Unknown mixing bowl", clean.group(1)
						else:
							print "Warning: Tried to clean mixing bowl", clean.group(1), "before putting anything in it!"
				continue
			...
			if verb != None:				
				if "until" in verb.group():
					continue
				if not self.ingredientlist.has_key(verb.group(2)):	# verb.group(2) is the ingredient
					continue
				if self.ingredientlist[verb.group(2)][0] == 0:
					...
					looptext = re.search(verb.group() + "\.((.*?)\s+[a-zA-Z]+ (?:(the )?([a-zA-Z ]+)) until " + verbw + "ed)", text, re.DOTALL|re.IGNORECASE)
					if not looptext:
						print 'Verb unmatched. Could not find "', verbw + 'ed" in "', text,'"'	# TODO - error reporting
